...
其中 `type` 字段的含义为：`0` 一次性提醒、`1` 每日提醒、`2` 工作日提醒（自动跳过周末、法定节假日与调休补班）。`priority` 字段的取值为 `0`（低）、`1`（中）、`2`（高）。

### 工作日数据

工作日提醒会优先读取程序目录下的 `workdays.json`，若不存在则使用内置的 `data/workdays.json`。文件结构示例：

```json
{
  "holidays": ["2024-10-01", "2024-10-02"],
  "makeupDays": ["2024-09-29"]
}
```

`holidays` 列表中的日期会被视为法定节假日，`makeupDays` 会被视作即使在周末也需要上班的调休补班日。你可以按照国家法定节假日安排在该文件中维护每年的数据，更新后重启程序即可生效。
